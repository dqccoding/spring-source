...
public abstract class AbstractHandlerMapping extends WebApplicationObjectSupport
	...
	public final HandlerExecutionChain getHandler(HttpServletRequest request) throws Exception {
		// 查找请求对应的Handler
		// 此处有两个实现类：AbstractUrlHandlerMapping和AbstractHandlerMethodMapping

		/**
		 * AbstractUrlHandlerMapping: 处理通过实现Controller接口或者HttpRequestHandler接口并通过@Component注入的Controller，也就是处理BeanName格式的Controller
		 * AbstractHandlerMethodMapping: 处理通过注解@Controller格式声明的处理器
		 */
		Object handler = getHandlerInternal(request);
		if (handler == null) {
			// 如果handler未找到，返回默认配置的handler
			handler = getDefaultHandler();
		}
		// 如果没有配置默认的handler，则直接返回null
		if (handler == null) {
			return null;
		}
		// Bean name or resolved handler?
		if (handler instanceof String) {
			String handlerName = (String) handler;se
